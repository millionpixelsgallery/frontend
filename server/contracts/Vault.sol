...
contract Vault is ERC20PresetFixedSupply {
    ...
    function withdraw(uint256 value) external {
        ...
        uint256 founderShare = (balanceOf(_msgSender()) * accumulatedEarnings) / totalSupply();
        require(founderShare >= value + founderDebt[_msgSender()], "value not available");
    ...
    function sell(uint256 amount) external {
        ...
        uint256 share = (payable(address(this)).balance * amount) / totalSupply();
    ...
    function buy(uint256 amount) external payable {
        require(active, "shares not active");
        uint256 share = (payable(address(this)).balance * amount * buyPremium) / (totalSupply() * 10000);

...
function Viewport({ className, style }: ViewportProps) {
  ...
  const [selectV, setSelectV] = useState<[x1: number, y1: number, x2: number, y2: number]>([
    10, 10, 110, 80,
  ])

  const [x1, y1, x2, y2] = selectV
      ...
      <ViewportContentSC ref={contentRef}>
        ...
        <Select
          ...
          onMove={(dx, dy) => {
            ...
            setSelectV([x1 + dx, y1 + dy, x2 + dx, y2 + dy])
          }}
          onResize={(dx, dy, [kx1, ky1, kx2, ky2]) => {
            setSelectV([
          ...
          onMouseUp={() => {
            setSelectV([Math.round(x1), Math.round(y1), Math.round(x2), Math.round(y2)])
          }}
        />
      </ViewportContentSC>
    </ViewportSC>
